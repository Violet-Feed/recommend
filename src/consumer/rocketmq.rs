...
use crate::handler::search_handler::handle_search_report;
                    ...
                    Ok(report) => {
                        ...
                        match report.report_type{
                            x if x == Embedding as i32 => {
                                if let Err(e) = handle_embedding_report(report).await {
                                    tracing::error!("[start] handle_embedding_report err. err = {:?}", e);
                                    continue;
                                }
                            }
                            x if x == Search as i32 => {
                                if let Err(e) = handle_search_report(report).await {
                                    tracing::error!("[start] handle_search_report err. err = {:?}", e);
                                    continue;
                                }
                            }
                            _ => {
